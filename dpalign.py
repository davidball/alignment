...
class AlignmentGraph():
    ...
    def compute_best_scored_path_for_each_node(self, g, m, n):
    ...
    def create_alignment_digraph(self, seq1, seq2):
        m = len(seq1)+1
        n = len(seq2)+1
        g = nx.DiGraph()
        ...
        self.compute_best_scored_path_for_each_node(g, m, n)

        return g


    # generate coordinates to position the nodes in a grid.
    def get_positions(self, m, n, grid_cell_size=10):
    ...
    def get_edges_in_best_paths(self, g, into_node_key):
        ...
        if 'best_predecessors' in sink:
            ...
            for pred in predecessors:
                edges = (edges + [(pred, into_node_key)] +
                         self.get_edges_in_best_paths(g, pred))
            return edges
        else:
            return []
    ...
    def get_best_paths(self, g, into_node_key):
        sink = g.node[into_node_key]
        if 'best_predecessors' in sink:
            predecessors = sink['best_predecessors']
            paths = []
            for pred in predecessors:
                parental_paths = self.get_best_paths(g, pred)
    ...
    def draw_traceback_indices(self, g, positions, seq1, seq2, cell_size):
        for j in range(0, len(seq1)+1):
            for k in range(0, len(seq2)+1):
                node = g.node[(j, k)]
                if "arrows" in node:
                    self.draw_traceback_arrow(node["arrows"],
                                         cell_size, positions[(j, k)])


    def get_traceback_arrow_codes(self, g, seq1, seq2):
        for j in range(0, len(seq1)+1):
            for k in range(0, len(seq2)+1):
                node = g.node[(j, k)]
    ...
    def draw_traceback_arrow(self, wingdingText, cell_size, pos, movie=None):
    ...
    def draw_sequence_letters(self, origin_position, cell_size, seq1, seq2):
    ...
    def draw_alignment_grid(self, g, seq1, seq2, filename="alignment.png",
                            plot=False,
                            movie=None):

        cell_size = 10
        positions = self.get_positions(len(seq1)+1, len(seq2)+1, cell_size)
        ...
        self.draw_sequence_letters(positions[(0, 0)], cell_size, seq1, seq2)
        ...
        self.get_traceback_arrow_codes(g, seq1, seq2)
        ...
        if movie.enabled:
            for k in range(len(seq2)+1):
                for j in range(len(seq1)+1):
                    ...
                    if "arrows" in n:
                        self.draw_traceback_arrow(n["arrows"], cell_size,
                                             positions[(j, k)])
                    plt.savefig(movie.nextImagePath())

        else:
            self.draw_traceback_indices(g, positions, seq1, seq2, cell_size)

        movie.saveFrame()

        movie.saveFrame()

        best_path_edges = self.get_edges_in_best_paths(g, (len(seq1), len(seq2)))
    ...
    def map_alignments(self, g, seq1, seq2):
        best_paths = self.get_best_paths(g, (len(seq1), len(seq2)))
    ...
    def display_alignments(self, seq1, seq2, alignments):
    ...
    def narration_text(self, seq1, seq2, alignments):
    ...
    def align_and_display(self, seq1, seq2, filename="", plot=False, make_movie=True):

        movie = MovieMaker("output/" + filename + ".", enabled=make_movie)

        g = self.create_alignment_digraph(seq1, seq2)

        self.draw_alignment_grid(g, seq1, seq2, filename, plot, movie)

        alignments = self.map_alignments(g, seq1, seq2)

        self.display_alignments(seq1, seq2, alignments)

        if make_movie:
            narration = self.narration_text(seq1, seq2, alignments)
